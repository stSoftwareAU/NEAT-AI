...
The new `Creature.discoveryDir()` helper allows external programs to schedule discovery work against a sampled dataset and receive the best candidate improvement plus a human-friendly summary.

Discovery requires the Rust FFI extension (`NEAT-AI-Discovery`). Always confirm it is available before starting:
...
if (!isRustDiscoveryEnabled()) {
  throw new Error("Discovery requires the NEAT-AI-Discovery Rust library and FFI permissions.");
}

const creature = Creature.fromJSON(existingCreatureJSON);

const discovery = await creature.discoveryDir(
  "./data/sample",
  {
...
if (discovery.improvement) {
  console.info(discovery.improvement.message);
  const improved = discovery.improvement.creature;
  // Score the improved creature or persist it for further evaluation.
}
```

`discoveryDir()` returns:

- `original`: baseline error and score for the current creature.
- `discovery`: the raw hints generated by the discovery worker.
- `improvement`: present when at least one candidate scores higher than the baseline, including:
  - `creature`: exported JSON for the best candidate.
  - `score`, `error`, and `scoreDelta`.
  - `changeType` plus a readable `message` describing what changed.

Multiple candidates (add/remove/change combinations) are scored in parallel using the worker pool so the external controller only needs to validate the best improvement over the full training dataset.
