...
The new `Creature.discoveryDir()` helper allows external programs to schedule
discovery work against a sampled dataset and receive the best candidate
improvement plus a human-friendly summary.

Discovery requires the Rust FFI extension (`NEAT-AI-Discovery`). Always confirm
it is available before starting:
...
if (!isRustDiscoveryEnabled()) {
  throw new Error(
    "Discovery requires the NEAT-AI-Discovery Rust library and FFI permissions.",
  );
}
...
- `improvement`: present when at least one candidate scores higher than the
  baseline, including:
...
Multiple candidates (add/remove/change combinations) are scored in parallel
using the worker pool so the external controller only needs to validate the best
improvement over the full training dataset.
