...
| `--gpu-scorer`                | Opt-in GPU smoke lane, added _after_ the default lanes (which keep `NEAT_SCORER_GPU=off` — GPU is **not** exercised by default). Runs `test/score/` at `DENO_JOBS=1` with `NEAT_SCORER_GPU=auto`. A pre-flight asks `rust_scorer` which backend it selected: no GPU is a clean skip, `cpu-fallback` fails loud. See [`docs/troubleshooting/CI.md`](docs/troubleshooting/CI.md). |
