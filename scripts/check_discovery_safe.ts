      ...
      if (ffiPermission.state === "granted") {
        ...
        try {
          ...
          if (isRustLibraryAvailable()) {
            ...
            console.error(
              "   - Architecture mismatch (check: file ~/.cargo/lib/libneat_ai_discovery.dylib)",
            );
            console.error(
              "   - Missing dependencies (run: otool -L ~/.cargo/lib/libneat_ai_discovery.dylib)",
            );
            console.error("   - Library built for different macOS version");
            console.error("   - Bug in library initialization");
            console.error(
              "   Solution: Rebuild the library in ../NEAT-AI-Discovery",
            );
            Deno.exit(1);
          }
        } catch (loadError) {
          ...
          console.error(
            "   - Library architecture mismatch (e.g., x86_64 vs arm64)",
          );
