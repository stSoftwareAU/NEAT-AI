 ...
 *
 * All of slice A's `QUALIFIES` verdicts have been carried out: `maxConns` and
 * `maximumNumberOfNodes` by #3552, `enableRepetitiveTraining` by #3553, and
 * `dnaSharingMode` by #3554. Those keys no longer exist in `NeatArguments`, so
 * the harness no longer enumerates them and their entries are gone from this
 * table. A retained entry would be an orphan (`reconcile()` reports keys the
