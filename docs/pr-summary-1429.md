...
(`__NEAT_AI_SKIP_WASM_AUTO_INIT` and `DEBUG`), causing `deno publish` to fail
with: _"modifying global types globals.d.ts:10:1"_.

Replaced `src/globals.d.ts` with `src/globalAccessors.ts` — a small utility
module that provides typed getter/setter functions for these two global flags
using an explicit `globalThis` cast, avoiding any `declare global` augmentation.
All call-sites updated to use the new accessors. Closes #1429.
