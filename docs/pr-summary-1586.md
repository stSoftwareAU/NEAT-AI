...
Replaced `Creature.fromJSON(creature.exportJSON())` with
`creature.shallowClone()` in `Mutator.mutate()` for cloning the original
creature before mutation. This avoids expensive JSON
serialisation/deserialisation when creating the backup used for memetic updates
and discovery. Closes #1586.
...
The `shallowClone()` approach is consistently 2-3.5x faster across all creature
sizes.
...
- Added `test/NEAT/MutatorShallowClone.ts` with 5 tests verifying:
  ...
  - `shallowClone()` produces structurally equivalent creatures (same UUID as
    JSON clone)
