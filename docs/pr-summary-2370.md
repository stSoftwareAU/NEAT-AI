...
pinned rev is required — justification recorded below and on the issue.
Closes #2370.
...
- `#2369` (wasm_activation audit) — two files (`topological_backprop.rs`,
  `topology_ops.rs`) still contain logic not yet in `neat-core` at the
  pinned rev; tracked upstream by `NEAT-AI-core#8` and `NEAT-AI-core#9`.
  Neither is yet landed upstream, so a bump cannot close either gap today.
- Upstream `stSoftwareAU/NEAT-AI-core@Develop` currently resolves to the
  same SHA as the pinned rev (GitHub compare API reports
  `ahead_by: 0, behind_by: 0, status: identical`). There is nothing newer
  to bump to.
...
- `[2/3]` Rust tests in `wasm_activation` against pinned `neat-core` —
  246 tests ok; `RUSTFLAGS=-D warnings` honoured.
...
- [ ] `./quality.sh` — N/A (no source changes beyond captured evidence; no
