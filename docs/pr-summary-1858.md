...
1. **Section 6 (Regularisation)**: Documented `WeightRegularisationConfig` and
   `BiasRegularisationConfig` with L2 regularisation support. Removed L1/L2
   from the "missing" list since L2 is now implemented.
...
3. **Training Methods & Unique Approaches**: Added Predictive Coding as a
   documented training paradigm (`PredictiveCodingConfig`) with its own
   unique approach section describing inference settling and Hebbian learning.
