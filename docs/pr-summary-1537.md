...
Replace `CreatureState.nodeMap` (`Map<number, NeuronState>`) with a
pre-allocated flat `NeuronState[]` array for O(1) neuron state access without
Map hashing overhead. Closes #1537.

### Changes

- **`src/architecture/CreatureState.ts`**: Replaced
  `nodeMap: Map<number, NeuronState>` with `nodeArray: NeuronState[]`,
  pre-allocated to `creature.neurons.length` at construction time.
