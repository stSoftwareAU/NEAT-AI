...
3. **Serialisation wall** — migrating the loop would require serialising the
   entire creature state (neurons, connections, sparse config, backprop
   buffers) to WASM memory per propagation call. Prior investigations (#1630,
   #1632) show serialisation costs 50–1,000x more than the computation saved
4. **Sequential dependency** — the loop is inherently sequential (each
   neuron's gradient depends on downstream results), limiting parallelisation
   gains even within WASM
