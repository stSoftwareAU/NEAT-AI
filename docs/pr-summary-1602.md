...
Three shared factory functions are provided in `test/fixtures/SimpleCreatures.ts`:
...
- Added `test/fixtures/SimpleCreatures.test.ts` with 4 tests verifying:
  ...
  - `makeBaseCreature` returns valid creature with 3 synapses including direct connection
  - `makeForwardOnlyCreature` returns valid 1-input, 1-output forward-only creature
