...
This PR implements the `derivative()` function for all 32 standard activation functions in Rust/WASM, completing Phase 6 of the WASM migration. This follows the DRY principle by providing a single source of truth for derivative calculations that can be used in backpropagation.

### Key Changes

1. **Rust Implementation** (`wasm_activation/src/lib.rs`):
   - Added `apply_derivative()` function with analytical derivative formulas for all 32 activation functions
...
2. **TypeScript Wrapper** (`src/wasm/WasmActivation.ts`):
   - Added `wasmDerivative()` function as the TypeScript interface to the WASM derivative
...
4. **Test Suite** (`test/WasmDerivative.ts`):
   ...
   - Uses floating-point tolerance (1e-4) to account for f32/f64 precision differences
...
This is a functional/API enhancement without UI changes. Evidence is provided through comprehensive test results:
...
- `test/WasmDerivative.ts` - 35 tests for WASM derivative functionality:
  ...
  - Comprehensive comparison test verifying all WASM derivatives match JS implementations
...
- [x] Tests verify WASM and JS derivatives match (within floating-point tolerance)
...
- Aggregate functions (MINIMUM, MAXIMUM, IF) return 0 as they don't have traditional derivatives
- StdInverse derivative handles x=0 specially to match JS `Math.sign(0) = 0` behaviour
- Complex derivatives (Mish, GELU) use larger tolerance (1e-3) due to complex formulas
