...
Replace O(n) linear scan with O(1) doubly-linked list + Map LRU in
`WasmCreatureActivationLRU.ts`. Closes #1534.

The previous implementation scanned all entries linearly to find the oldest for
eviction, and allocated a new `{ ref, lastAccess }` object on every call — even
for existing entries. This sits in the activation hot path (called on every
forward pass), scaling with population size × samples per generation.
