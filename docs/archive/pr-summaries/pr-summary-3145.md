...
against a pasted full-precision literal (`-0.49135010426905`) with no derivation
— the hard-coded-magic-value anti-pattern (check 5). A refactor of the fine-tune
blend that preserved its observable contract but computed intermediate values
differently would break the assertion even though behaviour was unchanged.
...
the fine-tune step blends the current fittest _towards_ the previous fittest,
...
- Both parent fixtures and the line-180 assertion now reference that constant
  instead of repeating the literal — so the assertion is **derived**, not
  pasted.
...
`deno fmt`, `deno lint`, and project-wide `deno check`
(`./quality.sh
--check-only`) all pass.
