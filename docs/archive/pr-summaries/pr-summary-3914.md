...
- **`docs/comparison/TRAINING_PARADIGMS.md`** gains a
  **🧭 Modern Gradient-Free Training** section between the NEAT-AI and
  reinforcement-learning sections, covering:
  ...
  - **Quality-diversity** — Lehman & Stanley (2011) novelty search and Mouret &
    Clune (2015) MAP-Elites: archives of behaviourally diverse elites rather than
    a single champion, with the NEAT-AI gap linked to `FUTURE_WORK.md`.
  ...
  - **Sample efficiency versus wall-clock** — the ES paper's own framing (less
    ...
    parallelism) applied to NEAT-AI's design: many cheap proposals, one expensive
    shared scoring judge. This reframes the "slower convergence" con as a
    deliberate, defended trade-off.
...
Documentation-only change with no web interface to screenshot, so the evidence is
the new structural test suite plus the full quality gate.
...
missing scoreboard, no hub link), which is the regression linkage for this issue.
...
- Existing `test/docs/ComparisonSplit.ts` continues to verify every relative link
  in the changed docs resolves on disk.
