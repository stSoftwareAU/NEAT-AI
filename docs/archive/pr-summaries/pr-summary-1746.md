...
1. `src/multithreading/workers/deno/worker.ts` (3 occurrences →
   `toErrorMessage`)
...
4. `src/multithreading/workers/WorkerProcessor.ts` (1 occurrence →
   `toErrorMessage`)
...
- Added `test/utils/ErrorSerialisation.ts` with 13 tests covering:
  - `toErrorMessage` with Error instances, Error subclasses, strings, numbers,
    null, undefined, objects
  - `toError` with Error instances, Error subclasses, strings, numbers, null,
    undefined
