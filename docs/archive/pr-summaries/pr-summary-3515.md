...
recorded against, the release sidecar `wasm_activation-pkg.tar.gz.sha256` is
the *only* anchor a revision advance can use — but `--allow-unverified` still
let an advance proceed with **no** anchor at all, then wrote the unattested
download's own hash back into `deno.json` as if it were verified
(trust-on-first-use). An advance is exactly the moment a substituted release
asset would be adopted, so that pin attested nothing.
...
- **Rev advance, no anchor** → hard error, **regardless of
  ...
  and links the upstream `wasm-bundle.yml` workflow that publishes the
  sidecar.
- **Rev advance, sidecar present and matching** → proceeds unchanged (a
  mismatch already hard-errors via the existing `verify_tarball_sha256`).
...
- `test/scripts/BuildScriptContentHash.ts`:
  ...
  - Added `guard_unverified_extract blocks a revision advance even with
    --allow-unverified (issue #3515)`: no-anchor + rev-advance aborts with and
    without `--allow-unverified`; a matching sidecar anchor still proceeds.
...
- `test/scripts/BuildScriptRetry.ts`:
  ...
  - Rewrote the unattested-download test as
    ...
    --allow-unverified (issue #3515)`: asserts non-zero exit in both cases,
    the actionable error text (sidecar name, release tag, `--allow-unverified`
    note, workflow link), nothing extracted, and `deno.json` left unmutated.
