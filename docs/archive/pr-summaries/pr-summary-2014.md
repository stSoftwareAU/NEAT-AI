...
Audit and fix parallel merge operations to prevent invalid creatures. Closes
#2014.
...
immediately at the point of mutation, matching the pattern already used in other
compact operations in `CompactCreature.ts`.
...
check and the strict bridge detection (exactly 1 inbound + 1 outbound) already
prevent the identified vulnerability scenarios.

## Evidence

All 4999 tests pass (0 failed, 6 ignored). The integrity assertions run inline
during every merge operation and would throw immediately if a dangling reference
were produced.
