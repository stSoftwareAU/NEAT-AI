...
The new SHA `a1d282b36b6f3519aa1f3fc636f609c47dddb294` corresponds to the v5.0.0
release tag, which migrates the action runtime from Node 20 to Node 24 (upstream
PR #1084). Action pin remains a 40-char commit SHA per the project's
supply-chain policy.
...
- Latest upstream release confirmed via
  `gh api repos/actions/dependency-review-action/releases/latest`: `v5.0.0`,
  published 2026-05-08, runtime upgraded to Node 24.
...
- Confirmed no other references to the old SHA remain in `.github/workflows/`
  (only historical mentions in archived PR summaries).
