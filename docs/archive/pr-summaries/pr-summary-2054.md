...
- **`exportJSON()`** (new) produces output with both UUIDs and integer IDs. This
  is for internal hot paths (evolution, training, discovery, checkpointing).
...
- `CreatureSerialization.ts` adds `exportJSON()` and
  `buildCreatureExportJSON(creature, includeIds)` helper.
- `Creature.ts` exposes `exportJSON()` as an instance method.
- All ~50 internal `src/` callers migrated from `exportJSON()` to
  `exportJSON()`.
- All ~200 test files updated to use `exportJSON()` where integer IDs are
  accessed.
