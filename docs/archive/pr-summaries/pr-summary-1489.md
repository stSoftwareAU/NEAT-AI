...
- **Cargo PATH management**: Added `$HOME/.cargo/bin` to PATH in `quality.sh` so
  Rust tools are discoverable in non-login shells
...
passing test suite (3820 tests, 0 failures) including 12 new tests that validate
the shell script improvements.

## Test Plan

- Added `test/scripts/RustlibVersionCompare.ts` (9 tests): Exercises the
  `_version_ge` semver comparison function from `scripts/rustlib.sh` with equal,
  higher, lower, pre-release, and short version inputs
- Added `test/scripts/BashScriptSyntax.ts` (3 tests): Validates all `.sh` files
  pass `bash -n` syntax checking, verifies `scripts/rustlib.sh` is sourceable,
  and confirms `require_rust_tools` is exported as a function
