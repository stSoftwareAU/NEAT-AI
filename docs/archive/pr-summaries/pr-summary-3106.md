...
- **Closed feature** — Issue #1307 is `CLOSED` / `COMPLETED`, so the work is
...
- **No dynamic references** — a repo-wide search for `MutationStabilityTracker`,
  ...
  string-keyed or reflective use outside the deleted module, its test, and
  stale build logs.
...
package entry point — deleting both files leaves the rest of the library
(`D`) untouched.
...
- Ran `./quality.sh < /dev/null` (lint, type-check, full test suite):
  `7397 passed | 0 failed | 4 ignored`. No remaining references to the
  removed symbols in `src/`, `test/`, `scripts/`, `bench/`, or `mod.ts`.
