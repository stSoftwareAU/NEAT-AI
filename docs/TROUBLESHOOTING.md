...
- **`ScorerStrictError` during `deno test`** — strict mode is on by default, so
  a scorer failure aborts instead of falling back to WASM. →
...
| `NEAT_AI_RUST_SCORER_STRICT`      | `true`         | Throw on a scorer exec/parse failure; set `0` to fall back to WASM     |
