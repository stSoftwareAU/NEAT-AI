...
for memory pressure and WASM cache behaviour during fitness evaluation. Closes
#2263.
...
proactively clearing WASM caches when the heap is under pressure. This gives the
fitness phase more memory headroom and reduces scattered GC pauses during
`fitnessMs`.
...
3. **The existing graduated pressure response is well-tuned**: warning halves
   the activation LRU cap, critical clears all caches.
