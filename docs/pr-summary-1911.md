...
`shouldSkipRustDiscoveryTests()` function now returns
`!isRustDiscoveryEnabled()` directly, skipping tests gracefully when the library
is absent without requiring any environment variable. Closes #1911.
...
- All 4752 existing tests continue to pass (4 pre-existing failures unrelated to
  this change)
