...
- 26 squash/activation functions (IDENTITY, ReLU, TANH, LOGISTIC, SELU, ELU,
  ...
  GAUSSIAN, BENT_IDENTITY, BIPOLAR_SIGMOID, BIPOLAR, STEP, COMPLEMENT,
  ABSOLUTE, SQUARE, Cube, HARD_TANH, ReLU6)
...
All comparisons use a combined absolute (1e-8) and relative (1e-6) tolerance
to account for f32 vs f64 precision differences.
...
- The one pre-existing intermittent failure (`TraceNeuron.ts`) is unrelated
  to this change and passes when run in isolation
...
- Added `test/score/TsRustScorerConsistency.ts` with 21 test cases that:
  ...
  3. Score using TS pipeline (`Creature.fromJSON()` -> `activate()` -> cost -> `Score.calculate()`)
