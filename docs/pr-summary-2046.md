...
functions in `src/architecture/Score.ts`. These functions previously allocated new
`Float64Array` instances on every call in the score update hot path. Now they use
module-level pooled buffers that grow as needed (never shrink) and return subarray
views, following the proven pre-allocated buffer pattern from
