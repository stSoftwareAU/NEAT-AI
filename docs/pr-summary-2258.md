...
1. **String-based hashing** (`CreatureUtils.ts`): Replace object allocation +
   `JSON.stringify` with tab-delimited string keys for both neurons and
   synapses. Uses a flat `string[]` UUID lookup instead of
   `Map<number, string>`.
...
2. **Incremental neuron caching** (`Creature.ts`): Cache the sorted neuron
   ...
   connection-only invalidations (`clearConnectionCaches`, connection-only
   `clearCache`) but cleared on full topology invalidation (neuron add/remove).
