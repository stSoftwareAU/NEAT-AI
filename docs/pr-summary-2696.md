...
- `deno test --allow-read test/ci/WorkflowActionPinning.ts` — 7 passed, 0
  failed. The two new Issue #2696 tests scan every workflow file and assert that
  each `uses:` reference resolves to a 40-char lowercase hex SHA and is
