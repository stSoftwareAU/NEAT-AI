...
| **[Configuration](api/CONFIGURATION.md)**               | `NeatOptions` / `NeatOptionsInput`, every sub-config (plateau, stability, regularisation, MCMC, output range, adaptive population, parallel evaluation, data fuzzing, selection pressure), training-event callbacks, `Logger`, and the `RandomNumberGenerator`.         |
