...
Fixed to use the correct IDs (hidden-B: 1775329633, hidden-A: 1775329634) so
the synapse lookup succeeds. Closes #2031.

## Changes

- Removed `ignore: true` from the `Deno.test()` options in
  `test/discovery/DiscoveryScenarioFanInSynapsePatterns.ts`
- Fixed `hiddenBId` and `hiddenAId` to use deterministic UUID-based IDs
