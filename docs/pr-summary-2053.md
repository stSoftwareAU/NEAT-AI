...
Add explicit external interface contract tests for `exportJSON()` output to prevent future silent breakage of fields consumed by external systems. Closes #2053.

Issue #2049 identified that external interface fields (`uuid`, `fromUUID`, `toUUID`) were silently removed without test failures. These contract tests serve as a tripwire — each test names the specific field it guards, so any removal or rename immediately surfaces as a clearly labelled test failure.

## Evidence

All 15 new contract tests pass. Full quality gate passes with 5076 tests (0 failures).
