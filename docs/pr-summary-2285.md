...
iterations × full neuron scan inner) with per-neuron
`computeNeuronDependencyIndex` calls and collision-shifting. The new
implementation builds an adjacency list and in-degree map from the
`connectionsMap`, then processes neurons in BFS order with parent-index-based
