...
- `SubSelfCon.ts`: Removed 1 stress test that was near-identical to the
  existing stress test (same topology, same assertions)

**Misleading test name fixed:**
- `ModBiasRegularisation.ts`: Renamed "returns false when no valid neurons
  exist (with config)" to "skips constant neurons and mutates output neuron
  (with config)" — the test actually asserts mutation succeeds
