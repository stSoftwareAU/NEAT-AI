...
Added a forward-only synapse guard to `addHelpfulSynapses()` in `DiscoverySynapseOps.ts`. The function now rejects backward and self-loop synapses for forward-only creatures before they are pushed to `exportJSON.synapses`, preventing the downstream `loadFrom` stripping messages that indicated upstream corruption. Closes #2150.

## Evidence

Before the fix, `addHelpfulSynapses()` would add invalid synapses that were then stripped by `loadFrom` with:
```
...
- Added `test/ErrorGuidedStructuralEvolution/ForwardOnlySynapseGuardDiscovery.ts` with 4 tests:
