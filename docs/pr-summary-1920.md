...
Implement `computeLayerAssignments()` to compute neuron layer/level assignments from network topology. Each neuron is assigned a layer representing its depth from input neurons (longest path). This is the foundation for generating synthetic synapses between adjacent layers. Closes #1920.

- Input and constant neurons are assigned to layer 0
- Hidden neurons are assigned layers based on longest path from inputs using Kahn's algorithm
...
- Handles edge cases: disconnected neurons, skip connections, cycles, no hidden neurons
