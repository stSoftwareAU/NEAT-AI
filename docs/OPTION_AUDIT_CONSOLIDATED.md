...
```mermaid
flowchart LR
    H["#3518 harness<br/>enumerateOptionKeys()"] --> INV["266 rows<br/>112 top-level + 154 nested"]
...
is **112** today, as executed removals have taken keys back out. See
...
**`mutation` is `IN USE`.** The downstream production consumer sets it in four
`NeatOptions` literals — `src/exchange/EvolveApp.ts:418`,
`src/fx/EvolveApp.ts:365`, `src/industry/EvolveApp.ts:415` and
`src/location/EvolveApp.ts:471` — from its `--mutation=ALL|FFW` operator flag.
No removal issue is needed and no follow-up is filed, because the gap closed as
a live option rather than a missed candidate.
...
| #3559 | `novelty`                          | C     | Keep-or-remove decision resolved as remove; engine and 6 fields gone. |
| #3554 | `dnaSharingMode`                   | A     | Knob preset and `KnobTuningStrategy` gone; knob defaults inlined.     |
...
| C         |            59 |      38 | `adaptiveMutationThresholds` is declared inline in `NeatArguments`, so its 3 fields are not separate nested rows.                                 |
| D         |            69 |      63 | Same for `plateauDetection`'s 6 inline fields.                                                                                                    |
| E         |            33 |      37 | `rustScorer` gained a `NeatOptions` key in #3865; it has two interfaces, so `RequiredRustScorerConfig`'s 6 fields are enumerated alongside it.    |
| F         |            43 |      43 | —                                                                                                                                                 |
| roll-up   |             — |       1 | `mutation`.                                                                                                                                       |
| **Total** |       **286** | **266** |                                                                                                                                                   |

No slice lost a key in the merge; the deltas are enumeration conventions plus
the [executed removals](#executed-removals) — A is down 4 (#3552, #3553 and
#3554), B down 1 (#3556) and C down 18 (#3558 and its 10 nested fields, plus
#3559 and its 6 nested fields).
...
| `selection`                                   | A       | `IN USE`    | —     |                                                                                                                                 |
| `mutation`                                    | roll-up | `IN USE`    | —     | Gap found by this roll-up. The downstream consumer's four `EvolveApp.ts` entry points set it.                                   |
| `iterations`                                  | A       | `IN USE`    | —     |                                                                                                                                 |
| `verbose`                                     | A       | `IN USE`    | —     |                                                                                                                                 |
| `skipTrainingAfterConsecutiveRegressions`     | A       | `KEEP`      | —     |                                                                                                                                 |
| `skipTrainingAfterPopulationNoProgress`       | roll-up | `KEEP`      | —     | Added after the slices by #3779 — the population-wide no-progress training gate.                                                |
| `skipTrainingAfterPopulationRegressions`      | roll-up | `KEEP`      | —     | Added after the slices by a downstream scheduler — the stricter population-wide regressions-only gate.                          |
| `evaluationArchive`                           | roll-up | `KEEP`      | —     | Added after the slices by #3929 — the off-by-default append-only evaluation archive.                                            |
| `evolutionControl`                            | roll-up | `KEEP`      | —     | Added after the slices by #3931 — the off-by-default model-management policy; `"none"` is exact everywhere.                     |
| `preSelection`                                | roll-up | `KEEP`      | —     | Added after the slices by #3932 — off-by-default offspring over-generation and screening; `ratio: 1` breeds exactly the budget. |
| `preSelection.uncertainty`                    | roll-up | `KEEP`      | —     | Added after the slices by #3933 — the surrogate uncertainty guard; on by default, reachable only with a surrogate screen.       |
| `trainingGainLog`                             | roll-up | `KEEP`      | —     | Added after the slices by #3934 — the off-by-default per-training-event record of realised local-search gain.                   |
...
| `predictiveCoding`                            | D       | `IN USE`    | —     | The downstream production consumer sets only `.enabled`; the other four defaults then drive inference.                          |
| `wasmCache`                                   | E       | `KEEP`      | —     | The downstream consumer's 50 `wasmCache` hits are its own unrelated cap — a false IN USE.                                       |
| `discoveryCache`                              | B       | `KEEP`      | —     |                                                                                                                                 |
| `discoveryDiskSpace`                          | B       | `KEEP`      | —     |                                                                                                                                 |
| `memory`                                      | E       | `IN USE`    | —     | The downstream production consumer sets `enabled` + `nativeBudgetBytes`; seven defaults are live.                               |
...
| `mcmc`                                        | F       | `QUALIFIES` | #3570 | Decision — the downstream consumer's evolution-mode sweep declares intent but never wires it.                                   |
...
| `squashBudget`                                | D       | `QUALIFIES` | #3563 | Decision, audit recommends KEEP — live lever, adoption is a downstream consumer task.                                           |
...
| `rustScorer.env`                              | E       | `KEEP`      | —     |                                                                                                                                 |
| `rustScorer`                                  | E       | `IN USE`    | —     | #3865 promoted it to a NeatOptions key; option beats NEAT_AI_RUST_SCORER_* beats default.                                       |
...
| --------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------- |
| `dnaSharingMode` / `DnaSharingPreset`   | A ↔ F  | Slice A filed **#3554**; slice F recorded the overlap and filed nothing. One issue, now landed.                       |
...
PR lands and a downstream consumer fails its own `deno check` / test workflow.
