              ...
              if (
                ...
                if (fromSquash.safeZoneAdjustment) {
                  ...
                  if (Math.abs(safeZoneAdj) < config.plankConstant) {
                    console.warn(
                      `Out of safe zone for neuron ${fromNeuron.uuid}, squash ${fromSquash.getName()}, safeZoneAdj: ${safeZoneAdj}, targetFromValue: ${targetFromValue.toPrecision(3)}, errorPerLink: ${errorPerLink.toPrecision(3)}, `,
                    );
                  }
                }
