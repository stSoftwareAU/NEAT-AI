 ...
 *
 * Probing costs forward passes, so the callers gate it on the only shape whose
 * exactness cannot be reasoned about from weight magnitudes — an `IF` neuron
 * reached by role-typed synapses. {@link hasRoleTypedIfStructure} and
 * {@link feedsIfNeuron} are that gate: a creature carrying no such `IF` pays one
 * scan of the neuron array and never activates.
 *
 * @module
 */
  ...
  for (let row = 0; row < expected.length; row++) {
    ...
    for (let o = 0; o < expectedRow.length; o++) {
      ...
      if (!Number.isFinite(after)) {
        return { ...violation, rows: expected.length };
      }
...
export function describeDeviation(deviation: BehaviourDeviation): string {
  ...
  } against an output scale of ${
    deviation.scale.toExponential(3)
  } over ${deviation.comparisons} comparisons`;
}
...
/**
 * Whether this export carries at least one `IF` neuron reached by a role-typed
 * synapse.
 *
 * This is the cheap gate in front of the probe. An `IF` routes on the role
 * (`condition` / `positive` / `negative`) each inbound synapse carries, and its
 * thresholds and leaf values ride as weights on shared bias-1 constants — a
 * neuron contributing ~0 to any activation *sum*. That is the one shape where
 * a fold's exactness cannot be established from weight magnitudes, so it is the
 * one shape worth spending forward passes on. Returns before touching the
 * synapse array when the creature has no `IF` at all.
 */
export function hasRoleTypedIfStructure(
  creatureExport: CreatureExport,
): boolean {
  let ifIds: Set<number> | undefined;
  let ifUuids: Set<string> | undefined;
  for (const neuron of creatureExport.neurons) {
    if (neuron.squash !== IF.NAME) continue;
    if (neuron.id !== undefined) (ifIds ??= new Set()).add(neuron.id);
    if (neuron.uuid !== undefined) (ifUuids ??= new Set()).add(neuron.uuid);
  }
  if (!ifIds && !ifUuids) return false;

  for (const synapse of creatureExport.synapses) {
    if (synapse.type === undefined) continue;
    if (synapse.toId !== undefined && ifIds?.has(synapse.toId)) return true;
    if (synapse.toUUID !== undefined && ifUuids?.has(synapse.toUUID)) {
      return true;
    }
  }
  return false;
...
/**
 * Whether `neuronUuid` feeds any `IF` neuron in this export — the gate in front
 * of a discovery removal. A neuron feeding an `IF` supplies a routing decision
 * rather than an additive contribution, so a contribution-based impact metric
 * cannot see what deleting it costs.
 */
export function feedsIfNeuron(
  creatureExport: CreatureExport,
  neuronUuid: string,
): boolean {
  const ifUuids = new Set<string>();
  for (const neuron of creatureExport.neurons) {
    if (neuron.squash === IF.NAME && neuron.uuid !== undefined) {
      ifUuids.add(neuron.uuid);
    }
  }
  if (ifUuids.size === 0) return false;
