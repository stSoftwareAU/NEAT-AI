...
export function compactCreature(
  ...
  if (didCompact) {
    ...
    try {
      ...
      Deno.writeTextFileSync(".compacted-error.json", JSON.stringify(compactCreature, null, 1));
      c.fix();
      c.validate();
      Deno.writeTextFileSync(".compacted-fixed.json", JSON.stringify(c.exportJSON(), null, 1));
    }
