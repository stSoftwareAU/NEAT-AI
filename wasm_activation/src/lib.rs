...
/// Issue #1209 - SIMD-optimized weighted sum for 8 records simultaneously.
///
...
#[target_feature(enable = "simd128", enable = "relaxed-simd")]
#[inline]
unsafe fn weighted_sum_simd_8records(
    synapses: &[SynapseData],
    act0: &[f32],
    act1: &[f32],
    act2: &[f32],
    ...
    // Initialize accumulators with bias for all 8 records (two SIMD vectors)
...
fn apply_squash(squash_type: SquashType, x: f32) -> f32 {
    match squash_type {
        ...
        // Match TypeScript behavior (SELU clamps upper bound to avoid exp overflow).
        SquashType::Selu => {
            if !x.is_finite() {
        ...
        // Match TypeScript behavior (Softplus clamps at x>=709 to 100, non-finite -> 1e-15).
        SquashType::Softplus => {
            if !x.is_finite() {
                return 1e-15;
            }
...
fn apply_squash_f64(squash_type: SquashType, x: f64) -> f64 {
    match squash_type {
        ...
        SquashType::Relu => x.max(0.0),
        SquashType::Relu6 => x.max(0.0).min(6.0),
        SquashType::LeakyRelu => if x >= 0.0 { x } else { (LEAKY_RELU_ALPHA as f64) * x },
        SquashType::Selu => {
            // Match TypeScript behavior: clamp upper bound to avoid exp overflow.
            if !x.is_finite() {
        ...
        SquashType::Swish => {
            // Match TypeScript behavior: exp(-x) underflows for large +x.
            if !x.is_finite() {
                return 0.0;
            }
        ...
        SquashType::StdInverse => {
            ...
            let safe_x = if x.abs() < 1e-15 { if x > 0.0 { 1e-15 } else { -1e-15 } } else { x };
        ...
        SquashType::LogSigmoid => {
            // Match TypeScript clamp behavior.
            if !x.is_finite() || x <= -709.0 {
                return -(JS_MAX_SAFE_INTEGER as f64);
            }
            let exp_neg_x = (-x).exp();
            ...
            if neuron.is_constant {
                ...
                match squash {
                    SquashType::Minimum | SquashType::Maximum | SquashType::If => {
                        // Fall back to scalar for special squash functions
                        for (r, act) in [
                            (0, &mut act0), (1, &mut act1), (2, &mut act2), (3, &mut act3),
                            (4, &mut act4), (5, &mut act5), (6, &mut act6), (7, &mut act7),
                        ] {
                            let _ = r;
                            let activation = match squash {
                                SquashType::Minimum => {
                    ...
                    _ => {
                        ...
                        #[cfg(not(target_arch = "wasm32"))]
                        let (sum0, sum1, sum2, sum3, sum4, sum5, sum6, sum7) = weighted_sum_simd_8records(
                            &network.synapses,
                            &act0, &act1, &act2, &act3,
                            &act4, &act5, &act6, &act7,
                            start_synapse,
...
fn mae_sum_batch_8way(
    ...
    let mae_error = |records: &[f32], target_base: usize, act: &[f32], output_start: usize, num_outputs: usize| -> f64 {
    ...
    batch_8way_activation!(network, records, values_per_record, input_size, num_outputs, num_records, mae_error)
}
...
fn cross_entropy_sum_batch_8way(
    ...
    let ce_error = |records: &[f32], target_base: usize, act: &[f32], output_start: usize, num_outputs: usize| -> f64 {
    ...
    batch_8way_activation!(network, records, values_per_record, input_size, num_outputs, num_records, ce_error)
}
...
fn mape_sum_batch_8way(
    ...
    let mape_error = |records: &[f32], target_base: usize, act: &[f32], output_start: usize, num_outputs: usize| -> f64 {
    ...
    batch_8way_activation!(network, records, values_per_record, input_size, num_outputs, num_records, mape_error)
}
...
fn msle_sum_batch_8way(
    ...
    let msle_error = |records: &[f32], target_base: usize, act: &[f32], output_start: usize, num_outputs: usize| -> f64 {
    ...
    batch_8way_activation!(network, records, values_per_record, input_size, num_outputs, num_records, msle_error)
}
...
fn hinge_sum_batch_8way(
    ...
    // Note: No averaging per record to match JS implementation
    let hinge_error = |records: &[f32], target_base: usize, act: &[f32], output_start: usize, num_outputs: usize| -> f64 {
    ...
    batch_8way_activation!(network, records, values_per_record, input_size, num_outputs, num_records, hinge_error)
}
