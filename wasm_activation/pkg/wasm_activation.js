...
export class CompiledNetwork {
    ...
    to_dot(num_outputs) {
        ...
        let deferred1_1;
        try {
            const ret = wasm.compilednetwork_to_dot(this.__wbg_ptr, num_outputs);
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
     ...
     * @returns {string}
     */
    to_topology_json(num_outputs) {
        let deferred1_0;
        let deferred1_1;
        try {
            const ret = wasm.compilednetwork_to_topology_json(this.__wbg_ptr, num_outputs);
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
            return getStringFromWasm0(ret[0], ret[1]);
        } finally {
 ...
 * @returns {string}
 */
export function creature_validate(request) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ptr0 = passStringToWasm0(request, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.creature_validate(ptr0, len0);
        deferred2_0 = ret[0];
        deferred2_1 = ret[1];
        return getStringFromWasm0(ret[0], ret[1]);
    } finally {
