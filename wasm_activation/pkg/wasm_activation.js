 ...
 * @param {number} max_weight_adj_scale
 * @param {number} limit_weight_scale
 * @param {number} target_pre_activation
 * @param {number} pre_activation
 * @param {number} current_bias
 * @param {number} max_bias_adj_scale
 * @param {number} limit_bias_scale
 * @returns {Float64Array}
 */
export function fused_backprop_neuron(current_weights, target_values, activations, plank_constant, learning_rate, max_weight_adj_scale, limit_weight_scale, target_pre_activation, pre_activation, current_bias, max_bias_adj_scale, limit_bias_scale) {
    const ptr0 = passArrayF64ToWasm0(current_weights, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passArrayF64ToWasm0(target_values, wasm.__wbindgen_malloc);
    const len1 = WASM_VECTOR_LEN;
    const ptr2 = passArrayF64ToWasm0(activations, wasm.__wbindgen_malloc);
    const len2 = WASM_VECTOR_LEN;
    const ret = wasm.fused_backprop_neuron(ptr0, len0, ptr1, len1, ptr2, len2, plank_constant, learning_rate, max_weight_adj_scale, limit_weight_scale, target_pre_activation, pre_activation, current_bias, max_bias_adj_scale, limit_bias_scale);
    var v4 = getArrayF64FromWasm0(ret[0], ret[1]).slice();
    wasm.__wbindgen_free(ret[0], ret[1] * 8, 8);
    return v4;
}
