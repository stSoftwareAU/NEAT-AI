...
Deno.test("CRISPR", () => {
  ...
  const expectedTXT = JSON.stringify(
    clean(Creature.fromJSON(expectedJSON).exportInternalJSON()),
    null,
    2,
  );

  Deno.writeTextFileSync("test/data/CRISPR/.expected-IF.json", expectedTXT);
  const actualJSON = (networkIF as Creature).exportInternalJSON();
...
Deno.test("CRISPR_twice", () => {
  ...
  const actualTXT = JSON.stringify(
    clean((networkIF2 as Creature).exportInternalJSON()),
    null,
    2,
  );
...
Deno.test("CRISPR-Volume", () => {
  ...
  const actualTXT = JSON.stringify(
    clean((networkIF as Creature).exportInternalJSON()),
    null,
    2,
  );
...
Deno.test("CRISPR-multi-outputs1", () => {
  ...
  const actualTXT = JSON.stringify(
    clean(networkSANE.exportInternalJSON()),
    null,
    2,
  );
