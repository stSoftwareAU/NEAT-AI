...
Deno.test("finishUp: the wait logs once per credit, not once per poll (Issue #3823)", async () => {
...
Deno.test("finishUp: a wait with no credits logs nothing at all (Issue #3823)", async () => {
  const dataDir = createTestDataDir(2, 1);
  const workers = createTestWorkers(dataDir);
  let log = { lines: [] as string[], restore: () => {} };
