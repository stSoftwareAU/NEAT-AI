...
Deno.test("buildDiscoveryCandidates returns empty list when there are no suggestions", () => {
  ...
  const originalSynapseCount = base.exportInternalJSON().synapses.length;
  ...
  () => {
    ...
    const findDedicatedCandidate = (target: CandidateSynapse) => {
      return addSynapseCandidates.find((candidate) => {
        const exported = candidate.creature.exportInternalJSON();
    ...
    const firstExport = first.creature.exportInternalJSON();
    ...
    const secondExport = second.creature.exportInternalJSON();
...
Deno.test(
  ...
  () => {
    ...
    const exported = comboBest.creature.exportInternalJSON();
...
Deno.test("buildDiscoveryCandidates includes helpful neuron suggestions", () => {
  ...
  const exported = addedNeuron.creature.exportInternalJSON();
...
Deno.test(
  ...
  () => {
    ...
    const slotA = addNeuronCandidates.filter((c) =>
      c.change.neuronDetails?.fromNeuronUuid === "hidden-1" &&
      c.change.neuronDetails?.toNeuronUuid === "output-0"
    );
    assertEquals(
      slotA.length,
      2,
    ...
    const pruned = pruneSuccessfulCandidatesForCombos([
      ...
      { candidate: slotA[1], scoreDelta: 0.002 },
      {
        candidate: addNeuronCandidates.find((c) =>
          c.change.neuronDetails?.fromNeuronUuid === "hidden-2" &&
          c.change.neuronDetails?.toNeuronUuid === "output-1"
        )!,
        scoreDelta: 0.0005,
      },
    ]);
