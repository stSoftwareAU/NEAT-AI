...
Deno.test("NeuronErrorImpactEstimator splits share across inbound synapses", () => {
  ...
  const shareHiddenA = estimator.getNeuronShare("hidden-a" as unknown as number);
  const shareHiddenB = estimator.getNeuronShare("hidden-b" as unknown as number);
...
Deno.test("NeuronErrorImpactEstimator propagates recursively through multiple layers", () => {
  ...
  assertAlmostEquals(estimator.getNeuronShare("hidden-b" as unknown as number), 0.5, 1e-6);
  assertAlmostEquals(estimator.getNeuronShare("hidden-c" as unknown as number), 0.5, 1e-6);
  // hidden-a feeds both hidden-b and hidden-c, so it accumulates their shares
  assertAlmostEquals(estimator.getNeuronShare("hidden-a" as unknown as number), 1, 1e-6);
  // Inputs split hidden-a share proportional to absolute weights 2:1
  assertAlmostEquals(estimator.getNeuronShare("input-0" as unknown as number), 2 / 3, 1e-6);
  assertAlmostEquals(estimator.getNeuronShare("input-1" as unknown as number), 1 / 3, 1e-6);
});
...
Deno.test("NeuronErrorImpactEstimator averages contribution across multiple outputs", () => {
  ...
  assertAlmostEquals(estimator.getNeuronShare("input-0" as unknown as number), 1, 1e-6);
});
