import { assertAlmostEquals } from "https://deno.land/std@0.197.0/testing/asserts.ts";
...
Deno.test("correctInternal", () => {
  ...
  const json2 = Upgrade.correct(Network.fromJSON(json).internalJSON(), 5);
  const network = Network.fromJSON(json2);
  console.info(JSON.stringify(network.internalJSON(), null, 2));
  for (let p = 0; p < 1000; p++) {
    const a = Math.random() * 2 - 1;
    const b = Math.random() * 2 - 1;
    const c = Math.random() * 2 - 1;
