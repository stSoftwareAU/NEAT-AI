...
// ---------------------------------------------------------------------------
// Convergence across diverse starting biases (moved from AccumulateBias.ts)
// ---------------------------------------------------------------------------

function makeMultiOutputCreature(): Creature {
  /*
   *  i0 i1 i2
...
Deno.test("Bias convergence - adjustedBias converges to target across diverse starting biases", () => {
  let config: BackPropagationConfig = createBackPropagationConfig({
    generations: 0,
    learningRate: 1,
    batchSize: 1,
  });

  const creature = makeMultiOutputCreature();
  ...
  biases.forEach((bias) => {
    ...
    assertAlmostEquals(
      ...
      bias,
      tolerance,
      `aBias: ${aBias}, bias: ${bias}, tolerance: ${tolerance}`,
    );
  });
});
