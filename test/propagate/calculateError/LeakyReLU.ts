...
Deno.test("LeakyReLU.calculateError: standard positive cases", () => {
  ...
  assertEquals(leaky.calculateError(1.0, 1.0), 0.0); // no error
  assertEquals(leaky.calculateError(0.5, 1.0), 0.5); // positive delta
  assertEquals(leaky.calculateError(1.0, 0.0), -1.0); // decreasing
});
...
Deno.test("LeakyReLU.calculateError: corner cases", () => {
  ...
  // both zero
  assertEquals(leaky.calculateError(0.0, 0.0), 0.0);

  // zero target
  assert(Number.isFinite(leaky.calculateError(1.0, 0.0)));

  // zero current
  assert(Number.isFinite(leaky.calculateError(0.0, 1.0)));
