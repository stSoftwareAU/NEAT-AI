/**
 * Issue #1518 - WASM Batch Weight/Bias Accumulation Performance Benchmark
 *
 * Compares TypeScript vs WASM implementations of batch weight and bias
 * accumulation functions. The WASM implementation moves the entire accumulation
 * loop into Rust, eliminating JS/WASM boundary crossings per synapse/neuron.
 *
 * Run with:
...
console.log("\n" + "=".repeat(70));
console.log("Issue #1518: WASM Batch Accumulation Performance Benchmark");
...
Deno.bench({
  name: "Weight: 4-way batch (WASM+fallback)",
  group: "weight-accumulation",
}, () => {
  const states: SynapseState[] = [];
  for (let i = 0; i < BATCH_SIZE; i++) {
...
Deno.bench({
  name: "Weight: 8-way batch (WASM+fallback)",
  group: "weight-accumulation",
}, () => {
  const states: SynapseState[] = [];
  for (let i = 0; i < BATCH_SIZE; i++) {
...
Deno.bench({
  name: "Bias: 4-way batch (WASM+fallback)",
  group: "bias-accumulation",
}, () => {
  const states: NeuronState[] = [];
  for (let i = 0; i < BATCH_SIZE; i++) {
...
Deno.bench({
  name: "Bias: 8-way batch (WASM+fallback)",
  group: "bias-accumulation",
}, () => {
  const states: NeuronState[] = [];
  for (let i = 0; i < BATCH_SIZE; i++) {
...
console.log(
  "Note: WASM batch functions move entire accumulation loops to Rust,",
);
console.log("eliminating JS/WASM boundary crossings per synapse/neuron.");
console.log("=".repeat(70) + "\n");
